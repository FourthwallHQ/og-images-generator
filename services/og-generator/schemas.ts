...
export const OGImageShopRequestSchema = z.object({
  ...
  offerImagesUrls: z.array(z.url()).optional().openapi({
    example: [
  ...
  poweredBy: z.boolean().optional().openapi({
    example: true,
    description: 'Show "Powered by Fourthwall" section (auto-set based on strategy if not provided)'
  }),
}).refine(
  (data) => {
    if (data.strategy === 'LIVE_WITH_PRODUCTS' && (!data.offerImagesUrls || data.offerImagesUrls.length === 0)) {
      return false
    }
